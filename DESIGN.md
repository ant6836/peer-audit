...
**Theme:** light · **Base reference:** ElevenLabs style — `DESIGN_example.md` + Refero 상세자료(<https://styles.refero.design/style/031056ff-7af1-46db-8daa-115f731c5d26>), 우리 프로젝트용으로 각색
...
| Slate | `#a59f97` | 3차 텍스트·아이콘 스트로크·약화 라벨 (통계 수치 등) |
| Gravel | `#777169` | 보조 본문·내비·캡션·서브헤딩 — 따뜻한 돌빛 회색 |
| Obsidian | `#000000` | 주요 텍스트·헤드라인·로고 워드마크. 에그셸 대비 20.5:1 고대비 |
...
| heading | 32px | 1.13 | -0.64px | Serif 300 |
| heading-lg | 36px | 1.17 | -0.72px | Serif 300 |
| display | 48px | 1.08 | -0.96px | Serif 300 |

> 트래킹은 EL 원본의 px 값(헤드라인 ≈ -0.02em). 모바일(≤600px)에선 display를 34px로 축소.
...
## Elevation
ElevenLabs는 **헤어라인 3단** 만 쓴다. 깊은 그림자/강한 엘리베이션 금지 — 모든 요소를 같은 평면에 둬 타입 우선의 의도를 지킨다. (CSS 변수로 구현)

| 토큰 | 값 | 용도 |
|---|---|---|
| `--shadow-inset` | `rgba(0,0,0,0.075) 0 0 0 0.5px inset` | 흰 입력·면 — 보더 대신 안쪽 헤어라인 (검색창) |
| `--shadow-2` | `rgba(0,0,0,0.06) 0 0 0 1px, rgba(0,0,0,0.04) 0 1px 2px, rgba(0,0,0,0.04) 0 2px 4px` | 버튼·드롭다운 |
| `--shadow-card` | `rgba(0,0,0,0.4) 0 0 1px, rgba(0,0,0,0.04) 0 2px 4px` | 카드 — 쌓이지 않고 *뜬다* |

> 카드 상한: `rgba(0,0,0,0.4) 0 0 1px` 보다 큰 box-shadow 금지.
